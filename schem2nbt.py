# https://github.com/FolfyBlue/Schem-File-to-Structure-Block-NBT-Format
import argparse
import logging
...
def process_block_palette(
    ...
    for _palette, block in byte_palette.items():
        block_name, block_properties = re.findall(
            r"(minecraft:\w+)(\[.+\])?", block)[0]
        ...
        if len(bp) > 0:
            nbt_schematic["palette"].append(
                {"Name": block_name, "Properties": bp})
        else:
...
def process_paths(args: argparse.Namespace) -> tuple[list[str], list[str]]:
    ...
    if args.folder:
        ...
        output_files = [
            args.output or f"{os.path.splitext(args.input)[0]}.nbt"]
